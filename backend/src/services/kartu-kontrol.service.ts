...
export async function createKartuKontrol(
  ...
  if (user.role === Role.ORANG_TUA) {
    ...
    throw new ApiError(403, "Role ADMIN tidak dapat input setoran hafalan");
  }
...
export async function updateKartuKontrol(
  ...
  if (ayatSelesai > existing.materi.totalAyat) {
    throw new ApiError(
      400,
      `ayatSelesai melebihi total ayat surah (${existing.materi.totalAyat})`
    );
  }

  let nilai = input.nilai !== undefined ? input.nilai : existing.nilai;
  if (user.role === Role.PENGAJAR && nilai !== null && nilai !== undefined) {
